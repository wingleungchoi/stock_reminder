...
def download_data_from_yahoo(page)
...
def collection_raw_data(stock_number)
...
def purify_data(stock_number) # only accepts a string of 4 digits as an argument. and return it pure data i.e. remove depulicates and make all elements string
  ...
  no_chinese_data = stocks_data_collection.map{|element| element.text.strip.gsub(/[年月]/, "-").gsub(/[日]/,'')}
...
(0..9).to_a.each do |stock_number_i| # create instances of the table DailyPrize
  stock_number = ''
  case stock_number_i
  when 0..8
  ...
  if !NUMBER_OF_MISSING_STOCK.include?(stock_number_i + 1)
    data = data_packed_in_day(stock_number)
    total_index_of_data = data.length - 1
    (0..total_index_of_data).each do |data_i|
      daily_prize = DailyPrice.new
      daily_prize.stock_number = stock_number_i + 1
      daily_prize.date = data[data_i][:date]
      daily_prize.open = data[data_i][:open].to_f
      daily_prize.high = data[data_i][:high].to_f
      daily_prize.low = data[data_i][:low].to_f
      daily_prize.close = data[data_i][:close].to_f
      daily_prize.trading_volume = data[data_i][:trading_volume].tr(",",'').to_i
      daily_prize.adjusted_close = data[data_i][:adjusted_close].to_f
      daily_prize.save
    end
  end
end
